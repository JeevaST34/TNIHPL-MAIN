...
import { resident, getToken, type ResidentAnnouncement } from "@/lib/resident";

const fmt = (d: string) => new Date(d).toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" });

export default function AnnouncementsPage() {
  const router = useRouter();
  const [items, setItems] = useState<ResidentAnnouncement[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setItems(await resident.announcements());
      } catch (e) {
          ...
          <div className="hero-split-wrap">
            <div className="hero-split-left">
              <span className="hero-tag-badge">
                <i className="fa-solid fa-newspaper"></i> RESIDENT NEWS
              </span>
              <h1 className="breadcrumbs-title">News &amp; Announcements</h1>
              <p className="hero-subtitle-text">
                Important updates, circulars, maintenance schedules &amp; notices from hostel operations.
              </p>
            </div>
          </div>
        </div>
